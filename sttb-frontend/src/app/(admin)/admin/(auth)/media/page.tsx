...
export default function AdminMediaPage() {
  ...
  const totalCount = mediaResponse?.totalCount ?? 0;
  ...
  return (
    <>
      <UploadMediaDialog
        isOpen={isUploadOpen}
        onCloseAction={() => setIsUploadOpen(false)}
      />
          ...
          <div className="flex flex-wrap gap-4 items-center">
            <div className="relative flex-1 min-w-[300px]">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#0A2C74]"
              />
            </div>
            <div className="flex items-center gap-1.5 overflow-x-auto pb-2 sm:pb-0 no-scrollbar">
              ...
              {filterTypes.map((t) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-colors shrink-0 ${filterType === t ? "bg-[#0A2C74] text-white shadow-sm" : "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700"}`}
                >
                  {t}
                </button>
              ))}
            ...
            <div className="flex items-center gap-1 ml-auto shrink-0">
              <button
                ...
                className={`p-2 rounded-lg transition-colors ${viewMode === "grid" ? "bg-gray-100 dark:bg-gray-800 text-[#0A2C74] dark:text-white" : "text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800"}`}
                title="Grid View"
              >
              ...
              <button
                ...
                className={`p-2 rounded-lg transition-colors ${viewMode === "list" ? "bg-gray-100 dark:bg-gray-800 text-[#0A2C74] dark:text-white" : "text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800"}`}
                title="List View"
              >
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 px-2 border-r border-gray-100 dark:border-gray-800">
              <input
                ...
                className="w-4 h-4 rounded accent-[#0A2C74] cursor-pointer"
              />
            ...
            <div className="flex items-center gap-2">
              {selected.size > 0 && (
                <button
                  onClick={() => setDeleteConfirm([...selected])}
                  className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-red-50 dark:bg-red-900/20 text-[#E62129] text-xs font-semibold hover:bg-red-100 transition-colors border border-red-100 dark:border-red-900/30 shadow-sm"
                >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {filtered.map((item) => {
              ...
              return (
                ...
                >
                  <div
                    className="relative aspect-video cursor-pointer bg-gray-100 dark:bg-gray-800/50 overflow-hidden"
                    onClick={() => toggleSelect(item.id)}
                  >
                    {item.type && (
                      <span
                        className={`shadow-lg absolute top-2 right-2 capitalize font-semibold text-[10px] px-2 py-1 rounded-full line-clamp-1 max-w-25 text-white truncate ${`${item.type === "video" ? "bg-primary text-white" : "bg-secondary text-white"} : "bg-accent"`} `}
                      >
                        {item.type}
                      </span>
                    )}
                    {item.thumbnailUrl ? (
                        ...
                        src={
                          item.thumbnailUrl
                            ? item.thumbnailUrl.startsWith("http")
                              ? item.thumbnailUrl
                              : `http://localhost:5001${item.thumbnailUrl}`
                            : item.url.startsWith("http")
                              ? item.url
                              : `http://localhost:5001${item.url}`
                        }
                        alt={item.title}
                        height={0}
                        width={0}
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <Icon
                          className="w-8 h-8"
                          style={{ color: typeColors[item.type] || "#0A2C74" }}
                        />
                      </div>
                    )}
                    {isSelected && (
                  ...
                  <div className="p-3">
                    <p
                      className="text-gray-900 dark:text-white text-xs font-semibold truncate"
                      title={item.title}
                    >
                      {item.title}
                    </p>
                    <div className="flex items-center justify-between mt-0.5">
                      ...
                      {item.tag && (
                        <span
                          className="text-[10px] text-[#0A2C74] bg-[#0A2C74]/10 dark:text-blue-300 dark:bg-blue-900/30 px-1.5 py-0.5 rounded-sm line-clamp-1 max-w-25 truncate"
                          title={item.tag}
                        >
                          #{item.tag}
                        </span>
                    ...
                    <div className="flex w-fit items-center gap-1.5 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className="p-1.5 bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 rounded-md text-gray-500 hover:text-[#0570CD] transition-colors"
                        title="Copy URL"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 bg-red-50 hover:bg-red-100 dark:bg-red-900/10 dark:hover:bg-red-900/30 rounded-md text-red-500 transition-colors"
                        title="Hapus"
                      >
                ...
                <tr className="border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/20">
                  <th className="w-12 px-4 py-3">
                    <input
                      ...
                      onChange={toggleSelectAll}
                      className="w-4 h-4 rounded accent-[#0A2C74]"
                    />
                  </th>
              ...
              <tbody>
                {filtered.map((item) => {
                  ...
                  return (
                    <tr
                      key={item.id}
                      className={`border-b border-gray-50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors ${selected.has(item.id) ? "bg-blue-50/30 dark:bg-blue-900/10" : ""}`}
                    >
                          ...
                          <div className="w-10 h-10 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800 flex-shrink-0 flex items-center justify-center border border-gray-200 dark:border-gray-700">
                            ...
                            item.type === "article" ? (
                              ...
                              <Icon
                                className="w-5 h-5"
                                style={{
                                  color: typeColors[item.type] || "#0A2C74",
                                }}
                              />
                            )}
                          </div>
                          <span className="text-gray-900 dark:text-white text-sm font-medium truncate max-w-48">
                            {item.title}
                          </span>
                        </div>
                      </td>
                      <td className="px-4 py-3 hidden sm:table-cell">
                        <span
                          ...
                          style={{
                            backgroundColor: `${typeColors[item.type]}10`,
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1.5">
                          <button
                            ...
                            className="p-1.5 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-500 hover:text-[#0570CD] transition-colors"
                          >
                            <Copy className="w-4 h-4" />
                          </button>
                          <button
                            onClick={() => setDeleteConfirm([item.id])}
                            className="p-1.5 rounded-lg border border-red-100 dark:border-red-900/30 hover:bg-red-50 dark:hover:bg-red-900/20 text-gray-400 hover:text-[#E62129] transition-colors"
                          >
