...
import { Plus, Search, Edit2, Trash2, Filter, Calendar, MapPin, Tag, CheckCircle, XCircle, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function AdminKegiatanPage() {
    ...
    const categoryNames = ["Semua", ...(categoriesData?.map((c) => c.name) ?? [])];
                ...
                <div className="flex flex-wrap gap-3">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {categoryNames.map((c) => (
                            <button
                                key={c}
                                onClick={() => { setActiveCategory(c === "Semua" ? "" : c); setPage(1); }}
                        ...
                        <tbody>
                            ...
                            {!isLoading && filtered.map((ev) => (
                                <tr key={ev.id} className="border-b border-gray-50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                                    ...
                                    <td className="px-4 py-3">
                                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${ev.isPublished ? "bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400" : "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400"}`}>
                                            {ev.isPublished ? <CheckCircle className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
