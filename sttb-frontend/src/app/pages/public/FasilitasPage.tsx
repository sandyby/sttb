  ...
  return (
    <section ref={ref} className="py-20 bg-gray-950">
      ...
      <AnimatePresence>
        {lightbox && (
          ...
          >
            ...
            >
              <img src={lightbox.url} alt={lightbox.label} className="w-full max-h-[80vh] object-contain rounded-2xl" />
              <div className="absolute top-3 right-3">
                <button onClick={() => setLightbox(null)} className="w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm flex items-center justify-center text-white transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>
              <div className="mt-3 text-center">
                <span className="text-white/80 text-sm">{lightbox.label}</span>
                <span className="ml-3 px-2 py-0.5 rounded-full text-xs font-medium" style={{ backgroundColor: `${TAG_COLORS[lightbox.tag]}30`, color: TAG_COLORS[lightbox.tag] }}>{lightbox.tag}</span>
              </div>
            </motion.div>
          </motion.div>
        )}
      ...
      <div className="max-w-7xl mx-auto px-4">
        {/* Section header */}
        <motion.div initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }} className="text-center mb-10">
          ...
          <p className="text-white/50 text-sm max-w-lg mx-auto">Potret nyata kehidupan dan fasilitas kampus — klik foto untuk memperbesar.</p>
        </motion.div>

        {/* Tag filters */}
        <motion.div initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.15, duration: 0.5 }}
          className="flex flex-wrap gap-2 justify-center mb-8">
          {galleryTags.map(tag => (
            <button key={tag} onClick={() => setActiveTag(tag)}
              className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-all ${activeTag === tag ? "bg-[#E62129] text-white shadow-lg shadow-red-900/30" : "bg-white/10 text-white/70 hover:bg-white/20 hover:text-white"}`}>
              {tag}
            </button>
          ))}
        </motion.div>
              ...
              {filtered.map((img, i) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-3">
                    ...
                    <span className="mt-1 self-start px-2 py-0.5 rounded-full text-xs font-medium" style={{ backgroundColor: `${TAG_COLORS[img.tag]}40`, color: "white", backdropFilter: "blur(4px)" }}>
                      {img.tag}
                    </span>
                  </div>
