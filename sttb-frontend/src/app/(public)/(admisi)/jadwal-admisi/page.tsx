...
import { admisiContact, admisiWaves, methodIcons } from "@/data/admisi-data";

export default function JadwalAdmisiPage() {
    const [activeWave, setActiveWave] = useState("III");

    const wave = admisiWaves.find((w) => w.id === activeWave)!;
                        ...
                        {admisiWaves.map((w) => (
                            <StaggerItem key={w.id}>
                                <button
                                    ...
                                    className={`w-full text-left rounded-xl border-2 p-5 transition-all hover:shadow-md ${activeWave === w.id
