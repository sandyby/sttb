        ...
        >
            <div className="relative">
                ...
                <div className="relative p-6 rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all duration-500">
                    ...
                    <p className="text-[#E62129] text-xs font-semibold mt-1 mb-2">{p.position}</p>
...
function PengurusCard({ p, index }: { p: any; index: number }) {
    ...
    return (
        <motion.div
            ...
            whileHover={{ y: -6 }}
        >
            <div className={`absolute inset-0 bg-gradient-to-br ${colorStyles.accentBg} opacity-100`} />
...
export default function PengurusYayasanPage() {
    ...
    const { dewanPembina, dewanPengurus, anggota } = useMemo(() => {
        ...
        return {
            dewanPembina: members.filter(m => m.category?.toLowerCase() === "pembina"),
            dewanPengurus: members.filter(m => m.category?.toLowerCase() === "pengurus"),
                                ...
                                <AnimatePresence>
                                    {visibleAnggota.map((member, i) => (
                                        ...
                                        >
                                            ...
                                            <span className="text-gray-800 dark:text-white text-sm font-medium">{member.name}</span>
                                        </motion.div>
                                    ))}
                                </AnimatePresence>
                            </div>
