    ...
    <div className="space-y-2">
      <div className="flex flex-wrap gap-1.5 min-h-[28px]">
        {tags.map((t) => (
          <span
            key={t}
            className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#0A2C74]/10 text-[#0A2C74] dark:bg-blue-900/30 dark:text-blue-300"
          >
            #{t}
            <button
              type="button"
              onClick={() => remove(t)}
              className="hover:text-[#E62129] ml-0.5"
            >
        ...
        <input
          ...
          className="flex-1 min-w-16 text-sm bg-transparent text-gray-700 dark:text-gray-300 focus:outline-none placeholder-gray-400"
        />
      </div>
      <div className="flex flex-wrap gap-1">
        {TAG_SUGGESTIONS.filter((t) => !tags.includes(t)).map((t) => (
          <button
            ...
            type="button"
            onClick={() => add(t)}
            className="px-2 py-0.5 rounded text-xs text-gray-400 bg-gray-100 dark:bg-gray-800 hover:bg-[#E62129]/10 hover:text-[#E62129] transition-colors"
          >
            +{t}
          </button>
        ))}
...
export function EventForm({
  ...
  const {
    ...
    defaultValues: {
      ...
      startTime: (initialData as any)?.startTime ?? "08:00",
  ...
  const timeValue = watch("startTime");
  ...
  const handleSave = (status: "draft" | "published") => {
    ...
    const submitHandler: SubmitHandler<EventFormValues> = async (data) => {
      try {
  ...
  return (
    <div className="max-w-4xl mx-auto pb-20">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <button
            ...
            className="p-2.5 rounded-xl border border-gray-200 dark:border-gray-800 text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
          >
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              {isEdit
                ? "Perbarui detail kegiatan akademik"
                : "Buat dan publikasikan jadwal kegiatan baru"}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={!!saving}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#E62129] hover:bg-[#c4131a] text-white text-sm font-medium transition-all shadow-lg shadow-red-500/20 disabled:opacity-50"
          >
            {saving === "publish" ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : (
      ...
      {isEdit && (
        <div className="flex items-center gap-3 p-4 rounded-2xl bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800">
          ...
          <p className="text-blue-700 dark:text-blue-300 text-sm">
            Mode Edit — perubahan akan menggantikan data kegiatan ini.
          </p>
          <span
            className={`ml-auto px-2 py-0.5 rounded-full text-xs font-semibold ${statusValue === "published" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
          >
      ...
      {/* Live preview */}
      {titleValue && (
        <div className="bg-gradient-to-r from-[#0A2C74] to-[#0570CD] rounded-2xl p-4 mb-2 text-white">
      ...
      {tab === "basic" && (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 space-y-5">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <Field label="Kategori" required error={errors.category?.message}>
              <div className="flex gap-2">
                <select
                  {...register("category")}
                  className={inputCls(errors.category?.message) + " flex-1"}
                >
            ...
            >
              <div className="flex gap-2">
                <input
                  type="date"
                  {...register("startDate")}
                  className={inputCls(errors.startDate?.message) + " flex-1"}
                />
                <input
                  type="time"
                  {...register("startTime")}
          ...
          <div>
            ...
            <div className="space-y-3">
              <Field
                label={isOnline ? "Platform / Link" : "Nama Venue"}
                required
              ...
              <Field
                label="Detail Lokasi"
                hint="Alamat lengkap, nomor ruangan, petunjuk jalan"
              >
                <textarea
          ...
          >
            <textarea
              ...
              rows={5}
              placeholder="Jelaskan kegiatan secara lengkap — tema, tujuan, pembicara, agenda..."
              ...
              <div className="flex items-center justify-between p-4 rounded-2xl bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    Aktifkan jika kegiatan memerlukan registrasi peserta
                  </p>
                </div>
                <button
                  type="button"
                  ...
                  style={{
                    width: 44,
                    height: 24,
                    backgroundColor: field.value ? "#22C55E" : "#D1D5DB",
                  }}
                >
                  <span
          ...
          {registrationOpen && (
            <div className="space-y-4">
              ...
              <Field
                ...
                hint="Kosongkan jika tidak ada batas maksimum"
              >
                <div className="relative">
          ...
          <div className="border-t border-gray-100 dark:border-gray-800 pt-5">
            ...
            <Field label="Email Kontak">
              <input
                ...
                placeholder="panitia@sttb.ac.id"
                className={inputCls()}
              />
            </Field>
          </div>
          ...
          <Field label="Gambar Sampul / Cover">
            {coverImageUrl ? (
              ...
              <div
                ...
                className={`rounded-xl border-2 border-dashed p-8 text-center cursor-pointer transition-colors ${isUploading ? "border-blue-300 bg-blue-50/50 dark:bg-blue-900/20" : "border-gray-200 dark:border-gray-700 hover:border-[#E62129]"}`}
              >
                {isUploading ? (
                  <>
                  ...
                  <>
                    ...
                    <p className="text-[#E62129] text-xs mt-2 font-medium">
                      Max. File Size 10MB
                    </p>
                  </>
                )}
          ...
          <div className="bg-gray-50 dark:bg-gray-800 rounded-2xl p-4 text-xs space-y-2">
            ...
            {[
              ...
              { label: "Lokasi", ok: !!locationValue },
              {
                label: "Deskripsi",
                ok: !!descriptionValue,
                extra: descriptionValue
                  ? `${descriptionValue.split(/\s+/).filter(Boolean).length} kata`
                  : "",
              },
          ...
          {TABS.map((t) => (
            <button
              ...
              style={{
                width: tab === t.id ? 20 : 8,
                height: 8,
                backgroundColor: tab === t.id ? "#E62129" : "#D1D5DB",
              }}
            />
          ))}
        </div>
        <div className="flex items-center gap-2">
          {tab !== "basic" && (
            <button
              type="button"
              onClick={() =>
                setTab(tab === "details" ? "registration" : "basic")
              }
              className="px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 text-sm hover:bg-gray-50 transition-colors"
            >
              ← Sebelumnya
            </button>
          )}
          {tab !== "details" ? (
            <button
              type="button"
              onClick={() =>
                setTab(tab === "basic" ? "registration" : "details")
              }
              className="px-4 py-2 rounded-xl bg-[#0A2C74] text-white text-sm font-medium hover:bg-[#081f52] transition-colors"
            >
              Selanjutnya →
            </button>
          ) : (
            <button
              type="button"
              onClick={() => handleSave(statusValue)}
              disabled={!!saving}
